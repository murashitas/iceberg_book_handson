...
本資料では、本書第 7 章の内容に沿って、ターミナル上で HiveQL をインタラクティブに実行するためのクエリサンプルを紹介しています。ハンズオン環境で HiveQL を実行するにあたって、最初に**Hive 環境のセットアップ**と**事前準備**を完了させる必要があります。**Hive でデータ処理を実行する**以降の節では、本書の節名に対応したサンプルクエリを紹介しており、本書を参照しながら Iceberg テーブルを操作するための様々な Hive クエリを実行できます。
